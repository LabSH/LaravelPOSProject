@if (isset($paginator) && $paginator->lastPage() > 1)
	...
	@if ($paginator->currentPage() > 1)		<!-- 처음, 이전 -->
		<li class="page-item">
			<a class="page-link" href="{{ $paginator->url(1) }}" aria-label="First">
				<span aria-hidden="true" style="color:rgb(75,160,181)">◀</span>
			</a>
		</li>
	...
	@if( !$isCurrentPage)
			<a class="page-link" href="{{ $paginator->url($i) }}" style="color:rgb(75,160,181)">{{ $i }}</a>
	@else
			<a class="page-link bg-gradient-info" href="#" style="color:white; border:none;">{{ $i }}</a>
	@endif
